	...
	<a-table :scroll="{ y: tableHeight }" style="margin-top: 10px;" bordered :dataSource="tableList.list" :pagination="rolePage" :columns="columns">
		<template #bodyCell="{ column, record }">
			...
			<template v-if="column.key === 'action'">
				<a-button v-permission="['sys:course:select']" style="margin-left: 15px;" @click="selectBtn(record)" type="primary">
					<template #icon>
						<edit-outlined />
					</template>
					选课
...
import { ref, } from 'vue'
import useSelectTable from '@/composable/course/useSelectTable';
import useSelectCourse from '@/composable/course/useSelectCourse';
//表格相关的操作
const { rolePage, tableHeight, tableList, listParm, columns,getList } = useSelectTable()

//选课，退课
const {selectBtn,returnBtn,searchBtn,resetBtn} = useSelectCourse(getList,listParm)
