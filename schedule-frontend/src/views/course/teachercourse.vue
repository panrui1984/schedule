	...
	<a-table :scroll="{ y: tableHeight }" style="margin-top: 10px;" bordered :dataSource="tableList.list" :pagination="rolePage" :columns="columns">
		<template #bodyCell="{ column, record }">
			...
			<template v-if="column.key === 'action'">
				<a-button v-permission="['sys:myteacher:export']" @click="exportBtn(record)" style="margin-left: 15px;" type="primary">
					<template #icon>
						<delete-outlined />
					</template>
					导出课表
...
import { ref, } from 'vue'
import useTeacherTable from '@/composable/course/useTeacherTable';
//表格相关的操作
const { rolePage, tableHeight, tableList, listParm, columns,getList,searchBtn,resetBtn,exportBtn } = useTeacherTable()
