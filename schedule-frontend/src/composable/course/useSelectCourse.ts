...
export default function useSelectCourse(getList: FuncList, listParm: ListParm) {
	//获取全局global
	...
	//选课
	const selectBtn = async (row: CourseModel) => {
		//信息确定
		const confirm = await global.$myconfirm('确定选择该课程吗?');
		if (confirm) {
			const res = await selectCourseApi({
